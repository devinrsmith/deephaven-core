...
def _table_reader_all(
    ...
    *,
    emitter: Callable[[Sequence[Column], jpy.JType], T],
    row_set: jpy.JType,
    prev: bool = False,
) -> T:
...
def _table_reader_all_dict(
    ...
    *,
    ...
    Raises:
        ValueError
    """
    _emitter = lambda col_defs, j_array: {
        col_def.name: (
            _column_to_numpy_array(col_def, j_array[i]) if to_numpy else j_array[i]
        )
...
def _table_reader_chunk(
    ...
    cols: Optional[Union[str, Sequence[str]]] = None,
    *,
    emitter: Callable[[Sequence[Column], jpy.JType], Iterable[T]],
    row_set: jpy.JType,
    chunk_size: int = 2048,
    prev: bool = False,
...
def _table_reader_chunk_dict(
    ...
    *,
    ...
    def _emitter(
        col_defs: Sequence[Column], j_array: jpy.JType
    ) -> Generator[Dict[str, np.ndarray], None, None]:
        yield {
...
def _table_reader_chunk_tuple(
    ...
    *,
    ...
    named_tuple_class = namedtuple(
        tuple_name, cols or [col.name for col in table.columns], rename=False
    )
...
def _table_reader_row_dict(
    ...
    def _emitter(
        ...
        make_dict = lambda values: {
            col_def.name: value for col_def, value in zip(col_defs, values)
        }
        mvs = [
