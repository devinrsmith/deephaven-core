...
public class CharArrayChunkHasher implements ChunkHasher {
    public static final CharArrayChunkHasher INSTANCE = new CharArrayChunkHasher();

    private static void hashInitial(ObjectChunk<char[], Values> values, WritableIntChunk<HashCodes> destination) {
    ...
    private static void hashSecondary(ObjectChunk<char[], Values> values, WritableIntChunk<HashCodes> destination) {
        for (int ii = 0; ii < values.size(); ++ii) {
    ...
    public int hashInitial(Object value) {
        return hashInitialSingle((char[]) value);
    }

    @Override
    public int hashUpdate(int existing, Object value) {
        return hashUpdateSingle(existing, (char[]) value);
    }
