...
public final class ObjectProcessorJsonValueFromString implements ObjectProcessor<String> {

    private final JsonFactory jsonFactory;
    private final ValueOptions opts;

    public ObjectProcessorJsonValueFromString(JsonFactory jsonFactory, ValueOptions opts) {
        this.jsonFactory = Objects.requireNonNull(jsonFactory);
        this.opts = Objects.requireNonNull(opts);
    }

    public NamedObjectProcessor<String> named() {
        // todo: on singular / empty path
        return NamedObjectProcessor.of(this,
                opts.paths().map(ObjectProcessorJsonValueFromString::toColumnName).collect(Collectors.toList()));
    }
    ...
    @Override
    public void processAll(ObjectChunk<? extends String, ?> in, List<WritableChunk<?>> out) {
        try {
    ...
    void processAllImpl(ObjectChunk<? extends String, ?> in, List<WritableChunk<?>> out) throws IOException {
