...
public class IntArrayChunkHasher implements ChunkHasher {
    public static final IntArrayChunkHasher INSTANCE = new IntArrayChunkHasher();

    private static void hashInitial(ObjectChunk<int[], Values> values, WritableIntChunk<HashCodes> destination) {
    ...
    private static void hashSecondary(ObjectChunk<int[], Values> values, WritableIntChunk<HashCodes> destination) {
        for (int ii = 0; ii < values.size(); ++ii) {
    ...
    public int hashInitial(Object value) {
        return hashInitialSingle((int[]) value);
    }

    @Override
    public int hashUpdate(int existing, Object value) {
        return hashUpdateSingle(existing, (int[]) value);
    }
