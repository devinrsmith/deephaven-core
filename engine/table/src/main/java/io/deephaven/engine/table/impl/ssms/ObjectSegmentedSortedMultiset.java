...
public final class ObjectSegmentedSortedMultiset implements SegmentedSortedMultiSet<Object>, ObjectVector {
    ...
    private int insertExistingIntoLeaf(WritableObjectChunk<Object, ? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, MutableInt wipos, int leafSize, Object [] leafValues, long [] leafCounts, Object maxInsert, boolean lastLeaf) {
        int rlpos = 0;
        Object nextValue;
        while (rlpos < leafSize && ripos < valuesToInsert.size() && (ObjectComparisons.leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
    ...
    private void validateInputs(ObjectChunk<Object, ? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts) {
        ...
        for (int ii = 1; ii < valuesToInsert.size(); ++ii) {
            ...
            Assert.assertion(
                    ObjectComparisons.lt(prevValue, curValue),
                    "ObjectComparisons.lt(prevValue, curValue)",
                    prevValue, "prevValue",
                    curValue, "curValue");
        }
        ...
        if (leafCount == 0) {
            ...
            for (int ii = 0; ii < leafCount; ++ii) {
                ...
                if (ii < leafCount - 1) {
                    ...
                    Assert.assertion(
                            ObjectComparisons.leq(lastValue, directoryValue),
                            "ObjectComparisons.lt(lastValue, directoryValue)",
                    ...
                    if (ii < leafCount - 2) {
                        ...
                        Assert.assertion(
                                ObjectComparisons.lt(directoryValue, nextDirectoryValue),
                                "ObjectComparisons.lt(directoryValue, nextDirectoryValue)",
                                directoryValue, "directoryValue",
                    ...
                    Assert.assertion(
                            ObjectComparisons.lt(directoryValue, nextFirstValue),
                            "ObjectComparisons.lt(directoryValue, nextFirstValue)",
                            directoryValue, "directoryValue",
...
//                }
    ...
    private static void validateLeaf(Object[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(
                    ObjectComparisons.lt(values[ii], values[ii + 1]),
                    "ObjectComparisons.lt(values[ii], values[ii + 1])",
                    (Object)thisValue, "values[ii]",
                    (Object)nextValue, "values[ii + 1]",
                    ii, "ii");
        }
        if (size > 0) {
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(
                        ObjectComparisons.geq(getMinObject(), destination.getMaxObject()),
                        "ObjectComparisons.geq(getMinObject(), destination.getMaxObject())");
            }
        }
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(
                        ObjectComparisons.leq(getMaxObject(), destination.getMinObject()),
                        "ObjectComparisons.leq(getMaxObject(), destination.getMinObject())");
            }
        }
