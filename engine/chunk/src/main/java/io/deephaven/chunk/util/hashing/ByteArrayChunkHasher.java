...
public class ByteArrayChunkHasher implements ChunkHasher {
    public static final ByteArrayChunkHasher INSTANCE = new ByteArrayChunkHasher();

    private static void hashInitial(ObjectChunk<byte[], Values> values, WritableIntChunk<HashCodes> destination) {
    ...
    private static void hashSecondary(ObjectChunk<byte[], Values> values, WritableIntChunk<HashCodes> destination) {
        for (int ii = 0; ii < values.size(); ++ii) {
    ...
    public int hashInitial(Object value) {
        return hashInitialSingle((byte[]) value);
    }

    @Override
    public int hashUpdate(int existing, Object value) {
        return hashUpdateSingle(existing, (byte[]) value);
    }
