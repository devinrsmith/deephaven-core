...
public final class PositionInputStream extends FilterInputStream {
     ...
     * the _resulting_ input stream. {@code in} is closed during {@link #close()}; as such, the caller must ensure that
     * closing {@code in} does _not_ close {@code channel}. To remain valid, the caller must ensure that the resulting
     * input stream isn't re-wrapped by any downstream code in a way that would adversely effect the position (such as
     * wrapping the resulting input stream with buffering).
     *
