...
# pins, and isolates Gradle's toolchain resolution to whatever this flake
...
let
  ...
  # in Nix) -- but it means a fresh `nix develop` still needs network
  # access for that one download. Since the exact version and checksum are
  # already pinned in gradle-wrapper.properties, we can fetch and unpack
  # that same file as a Nix derivation (reusing its checksum, not a new
  # trust decision) and pre-seed the wrapper's on-disk cache, so
  # `./gradlew` finds it already there and skips the download.
  ...
  # the consuming flake put on PATH are the *only* ones available in this
  # shell. Anything that requests another version (e.g. this repo's
  # nightly CI matrix, which runs with -PtestRuntimeVersion=17/25 -- see
  ...
  # whether or not you're in `nix develop`, writing there would silently
  ...
  isolatedHomeHook = ''
    ...
    shopt -u nullglob
    {
      ...
      # Only whatever JDK(s) the consuming flake provides are available as
      # a toolchain in this shell -- anything requesting another version
      # (e.g. this repo's nightly CI matrix, which runs with
      # -PtestRuntimeVersion=17/25) will fail with "no matching toolchain
      # found" here rather than downloading one.
