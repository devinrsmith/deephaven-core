...
class TableFactoryTestCase(BaseTestCase):
    ...
    def test_input_table(self):
        ...
        with self.subTest("from table definition"):
            ...
            self.assertEqual(
                append_only_input_table.value_names, [col.name for col in cols]
            )
            ...
            self.assertEqual(
                ...
                [col.name for col in cols if col.name != "String"],
            )
        ...
        with self.subTest("deletion on input table"):
            ...
            self.assertEqual(
                ...
                [
                    col.name
                    for col in cols
                    if col.name != "String" and col.name != "Double"
                ],
            )
            self.assertEqual(keyed_input_table.size, 2)
    ...
    def test_input_table_empty_data(self):
        ...
        with cm:
            t = time_table("PT1s", blink_table=True)
            it = input_table(
                {c.name: c.data_type for c in t.columns}, key_cols="Timestamp"
            )
            it.add(t)
