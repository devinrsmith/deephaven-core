...
public class QueryTableDistinctStaticAggTest extends QueryTableDistinctAggTestBase {
    @Override
    public Table distinct(boolean includeNullAndNans, char[] source) {
        return TableTools.newTable(TableTools.charCol(S1, source)).aggAllBy(AggSpec.distinct(includeNullAndNans))
                .ungroup(S1);
    }

    @Override
    public Table distinct(boolean includeNullAndNans, float[] source) {
        return TableTools.newTable(TableTools.floatCol(S1, source)).aggAllBy(AggSpec.distinct(includeNullAndNans))
                .ungroup(S1);
    }

    @Override
    public Table distinct(boolean includeNullAndNans, double[] source) {
        return TableTools.newTable(TableTools.doubleCol(S1, source)).aggAllBy(AggSpec.distinct(includeNullAndNans))
                .ungroup(S1);
    }
