...
@Immutable
@SimpleStyle
abstract class OffsetsEnd extends OffsetsBase {
    ...
    final Map<TopicPartition, Offset> offsets(KafkaConsumer<?, ?> client) {
        return topicPartitions(client, topic()).collect(Collectors.toMap(Function.identity(), tp -> Offset.end()));
    }
    ...
    final Map<TopicPartition, Offset> offsets(Map<String, List<PartitionInfo>> info) {
        return info.get(topic())
                ...
                .collect(Collectors.toMap(Function.identity(), tp -> Offset.end()));
    }
}
