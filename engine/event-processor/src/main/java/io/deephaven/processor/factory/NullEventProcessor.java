...
public class NullEventProcessor<T> implements EventProcessorFactory<T> {
    ...
    public static <X> EventProcessorFactory<X> of(List<EventProcessorStreamSpec> specs, Random random) {
        final List<BiConsumer<Stream, Coordinator>> out = new ArrayList<>(specs.size());
        for (EventProcessorStreamSpec spec : specs) {
    ...
    private final List<EventProcessorStreamSpec> specs;
    private final List<BiConsumer<Stream, Coordinator>> consumers;

    private NullEventProcessor(List<EventProcessorStreamSpec> specs, List<BiConsumer<Stream, Coordinator>> consumers) {
    ...
    public void writeNullsInto(Sink sink) {
        ...
        while (consumerIt.hasNext()) {
            ...
            consumerIt.next().accept(streamIt.next(), sink.coordinator());
        }
        if (streamIt.hasNext()) {
            throw new IllegalStateException();
        }
    ...
    private static int expectedSize(EventProcessorStreamSpec spec, Random random) {
    ...
    private static class ColumnOrientedNullProducer implements BiConsumer<Stream, Coordinator> {
        public static ColumnOrientedNullProducer of(EventProcessorStreamSpec spec, Random random) {
            ...
            return new ColumnOrientedNullProducer(columnOrientedNullAppender, consumers, random,
                    expectedSize(spec, random));
        }
        ...
        private ColumnOrientedNullProducer(ColumnOrientedNullAppender columnOrientedNullAppender,
                List<Consumer<Appender>> columnOrientedConsumers, Random random, int expectedSize) {
    ...
    private static class RowOrientedNullProducer implements BiConsumer<Stream, Coordinator> {
        ...
        public static RowOrientedNullProducer of(EventProcessorStreamSpec spec, Random random) {
            if (!spec.isRowOriented()) {
                throw new IllegalArgumentException();
            }
            final List<Type<?>> types = spec.outputTypes();
        ...
        private RowOrientedNullProducer(int expectedSize, List<Consumer<Appender>> rowOrientedConsumers,
                Random random) {
    ...
    private enum RowOrientedNullSetter implements Visitor<Consumer<Appender>>,
            PrimitiveType.Visitor<Consumer<Appender>>, GenericType.Visitor<Consumer<Appender>> {
        INSTANCE;
    ...
    private static class ColumnOrientedNullAppender implements Visitor<Consumer<Appender>>,
            PrimitiveType.Visitor<Consumer<Appender>>, GenericType.Visitor<Consumer<Appender>> {
        private long times;
