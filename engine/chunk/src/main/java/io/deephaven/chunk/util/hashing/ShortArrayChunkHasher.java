...
public class ShortArrayChunkHasher implements ChunkHasher {
    ...
    private static void hashSecondary(ObjectChunk<short[], Values> values, WritableIntChunk<HashCodes> destination) {
        for (int ii = 0; ii < values.size(); ++ii) {
    ...
    public int hashInitial(Object value) {
        return hashInitialSingle((short[]) value);
    }

    @Override
    public int hashUpdate(int existing, Object value) {
        return hashUpdateSingle(existing, (short[]) value);
    }
