...
final class RexNodeFilterAdapterImpl extends RexVisitorBase<Filter> implements RexNodeFilterAdapter {
    ...
    private static Filter or(List<Filter> filters) {
        if (filters.isEmpty()) {
            // is this possible w/ calcite?
    ...
    private static Filter and(List<Filter> filters) {
        if (filters.isEmpty()) {
            // is this possible w/ calcite?
    ...
    private Filter or(RexCall call) {
        return applyN(RexNodeFilterAdapterImpl::or, call);
    }

    private Filter and(RexCall call) {
        return applyN(RexNodeFilterAdapterImpl::and, call);
    }
