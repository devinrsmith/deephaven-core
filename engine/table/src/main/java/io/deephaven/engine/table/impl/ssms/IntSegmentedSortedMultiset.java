...
public final class IntSegmentedSortedMultiset implements SegmentedSortedMultiSet<Integer>, IntVector {
    ...
    private int insertExistingIntoLeaf(WritableIntChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, MutableInt wipos, int leafSize, int [] leafValues, long [] leafCounts, int maxInsert, boolean lastLeaf) {
        int rlpos = 0;
        int nextValue;
        while (rlpos < leafSize && ripos < valuesToInsert.size() && (IntComparisons.leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
    ...
    private void validateInputs(IntChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts) {
        ...
        for (int ii = 1; ii < valuesToInsert.size(); ++ii) {
            ...
            Assert.assertion(
                    IntComparisons.lt(prevValue, curValue),
                    "IntComparisons.lt(prevValue, curValue)",
                    prevValue, "prevValue",
                    curValue, "curValue");
        }
        ...
        if (leafCount == 0) {
            ...
            for (int ii = 0; ii < leafCount; ++ii) {
                ...
                if (ii < leafCount - 1) {
                    ...
                    Assert.assertion(
                            IntComparisons.leq(lastValue, directoryValue),
                            "IntComparisons.lt(lastValue, directoryValue)",
                    ...
                    if (ii < leafCount - 2) {
                        ...
                        Assert.assertion(
                                IntComparisons.lt(directoryValue, nextDirectoryValue),
                                "IntComparisons.lt(directoryValue, nextDirectoryValue)",
                                directoryValue, "directoryValue",
                    ...
                    Assert.assertion(
                            IntComparisons.lt(directoryValue, nextFirstValue),
                            "IntComparisons.lt(directoryValue, nextFirstValue)",
                            directoryValue, "directoryValue",
...
//                }
    ...
    private static void validateLeaf(int[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(
                    IntComparisons.lt(values[ii], values[ii + 1]),
                    "IntComparisons.lt(values[ii], values[ii + 1])",
                    (Integer)thisValue, "values[ii]",
                    (Integer)nextValue, "values[ii + 1]",
                    ii, "ii");
        }
        if (size > 0) {
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(
                        IntComparisons.geq(getMinInt(), destination.getMaxInt()),
                        "IntComparisons.geq(getMinInt(), destination.getMaxInt())");
            }
        }
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(
                        IntComparisons.leq(getMaxInt(), destination.getMinInt()),
                        "IntComparisons.leq(getMaxInt(), destination.getMinInt())");
            }
        }
