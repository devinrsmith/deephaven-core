...
final class Helpers {
    static void assertNextToken(JsonParser parser, JsonToken expected) throws IOException {
        ...
        if (actual != expected) {
            throw new IllegalStateException(String.format("Unexpected next token. expected=%s, actual=%s", expected, actual));
        }
    }
    ...
    static void assertCurrentToken(JsonParser parser, JsonToken expected) {
        ...
        if (actual != expected) {
            throw new IllegalStateException(String.format("Unexpected current token. expected=%s, actual=%s", expected, actual));
        }
    }
    ...
    static void assertNextTokenIsValue(JsonParser parser) throws IOException {
        ...
        if (!actual.isScalarValue() && !actual.isStructStart()) {
            throw new IllegalStateException(String.format("Unexpected next token. expected value type, actual=%s", actual));
        }
    }
}
