...
class PartitionedTableProxyImpl extends LivenessArtifact implements PartitionedTable.Proxy {
    ...
    @Override
    public PartitionedTable.Proxy aj(TableOperations<?, ?> rightTable, Collection<? extends JoinMatch> columnsToMatch, AsOfJoinMatch joinMatch, Collection<? extends JoinAddition> columnsToAdd) {
        return complexTransform(rightTable, (ct, ot) -> ct.aj(ot, columnsToMatch, joinMatch, columnsToAdd),
                columnsToMatch.stream().limit(columnsToMatch.size() - 1).collect(Collectors.toList()));
    }
