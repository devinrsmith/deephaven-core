...
public class BooleanArrayChunkHasher implements ChunkHasher {
    public static final BooleanArrayChunkHasher INSTANCE = new BooleanArrayChunkHasher();

    private static void hashInitial(ObjectChunk<boolean[], Values> values, WritableIntChunk<HashCodes> destination) {
    ...
    private static void hashSecondary(ObjectChunk<boolean[], Values> values, WritableIntChunk<HashCodes> destination) {
        for (int ii = 0; ii < values.size(); ++ii) {
    ...
    public int hashInitial(Object value) {
        return hashInitialSingle((boolean[]) value);
    }

    @Override
    public int hashUpdate(int existing, Object value) {
        return hashUpdateSingle(existing, (boolean[]) value);
    }
