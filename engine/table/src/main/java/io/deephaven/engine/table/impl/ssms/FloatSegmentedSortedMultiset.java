...
public final class FloatSegmentedSortedMultiset implements SegmentedSortedMultiSet<Float>, FloatVector {
    ...
    private int insertExistingIntoLeaf(WritableFloatChunk<? extends Values> valuesToInsert, WritableIntChunk<ChunkLengths> counts, int ripos, MutableInt wipos, int leafSize, float [] leafValues, long [] leafCounts, float maxInsert, boolean lastLeaf) {
        int rlpos = 0;
        float nextValue;
        while (rlpos < leafSize && ripos < valuesToInsert.size() && (FloatComparisons.leq(nextValue = valuesToInsert.get(ripos), maxInsert) || lastLeaf)) {
    ...
    private void validateInputs(FloatChunk<? extends Values> valuesToInsert, IntChunk<ChunkLengths> counts) {
        ...
        for (int ii = 1; ii < valuesToInsert.size(); ++ii) {
            ...
            Assert.assertion(
                    FloatComparisons.lt(prevValue, curValue),
                    "FloatComparisons.lt(prevValue, curValue)",
                    prevValue, "prevValue",
                    curValue, "curValue");
        }
        ...
        if (leafCount == 0) {
            ...
            for (int ii = 0; ii < leafCount; ++ii) {
                ...
                if (ii < leafCount - 1) {
                    ...
                    Assert.assertion(
                            FloatComparisons.leq(lastValue, directoryValue),
                            "FloatComparisons.lt(lastValue, directoryValue)",
                    ...
                    if (ii < leafCount - 2) {
                        ...
                        Assert.assertion(
                                FloatComparisons.lt(directoryValue, nextDirectoryValue),
                                "FloatComparisons.lt(directoryValue, nextDirectoryValue)",
                                directoryValue, "directoryValue",
                    ...
                    Assert.assertion(
                            FloatComparisons.lt(directoryValue, nextFirstValue),
                            "FloatComparisons.lt(directoryValue, nextFirstValue)",
                            directoryValue, "directoryValue",
...
//                }
    ...
    private static void validateLeaf(float[] values, long[] counts, int size) {
        ...
        for (int ii = 0; ii < size - 1; ++ii) {
            ...
            Assert.assertion(
                    FloatComparisons.lt(values[ii], values[ii + 1]),
                    "FloatComparisons.lt(values[ii], values[ii + 1])",
                    (Float)thisValue, "values[ii]",
                    (Float)nextValue, "values[ii + 1]",
                    ii, "ii");
        }
        if (size > 0) {
    ...
    public void moveFrontToBack(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(
                        FloatComparisons.geq(getMinFloat(), destination.getMaxFloat()),
                        "FloatComparisons.geq(getMinFloat(), destination.getMaxFloat())");
            }
        }
    ...
    public void moveBackToFront(SegmentedSortedMultiSet untypedDestination, long count) {
        ...
        if (SEGMENTED_SORTED_MULTISET_VALIDATION) {
            if (destination.size > 0) {
                Assert.assertion(
                        FloatComparisons.leq(getMaxFloat(), destination.getMinFloat()),
                        "FloatComparisons.leq(getMaxFloat(), destination.getMinFloat())");
            }
        }
