...
public class LongArrayChunkHasher implements ChunkHasher {
    public static final LongArrayChunkHasher INSTANCE = new LongArrayChunkHasher();

    private static void hashInitial(ObjectChunk<long[], Values> values, WritableIntChunk<HashCodes> destination) {
    ...
    private static void hashSecondary(ObjectChunk<long[], Values> values, WritableIntChunk<HashCodes> destination) {
        for (int ii = 0; ii < values.size(); ++ii) {
    ...
    public int hashInitial(Object value) {
        return hashInitialSingle((long[]) value);
    }

    @Override
    public int hashUpdate(int existing, Object value) {
        return hashUpdateSingle(existing, (long[]) value);
    }
